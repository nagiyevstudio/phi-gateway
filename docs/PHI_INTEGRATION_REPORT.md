...
### 3.3. Анализ фотографии чека (Vision OCR + классификация)
* **Эндпоинт**: `POST /phi/receipt/analyze`
* **Тело запроса**:
  ```json
  {
    "image_base64": "...", // изображение в base64 (jpeg/png)
  ...
  ```
* **Поведение шлюза**:
  1. Распознает текст чека с фотографии (`image_base64`) через модель `phi-vision` (OCR).
  2. Сопоставляет товары по регулярным выражениям из `category_rules`.
  3. Оставшиеся нераспознанные товары отправляет на классификацию в `phi-classifier`.
* **Ответ (200 OK)**:
...
### 3.4. Парсинг голосовых и текстовых транзакций
...
### 3.5. Пакетная классификация списка товаров
...
* `gateway_timeout` (HTTP 504) — таймаут ожидания провайдера.
